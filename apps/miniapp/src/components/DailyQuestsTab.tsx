...
import { CheckCircle2, Gift, Play, Star, AlertCircle } from 'lucide-react';
...
export function DailyQuestsTab({ profile, onProfileUpdated }: DailyQuestsTabProps) {
  // Mock quests for demo. In production, fetch these from DB based on UserProfile.dailyQuests
  const [quests, setQuests] = useState([
    { id: '1', title: "5 ta yangi so'z yodlang", target: 5, progress: 3, completed: false },
    { id: '2', title: "Bot bilan ovozli suhbatlashing", target: 1, progress: 1, completed: true },
    { id: '3', title: "1 ta hikoya o'qing", target: 1, progress: 0, completed: false }
  ]);
            ...
            <div key={q.id} className="bg-white/5 rounded-2xl p-4 border border-white/10 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-500 ${q.completed ? 'bg-green-400' : 'bg-indigo-400'}`}
